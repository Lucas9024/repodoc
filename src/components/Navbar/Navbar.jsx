...
<Topo />
      ...
      <div className='mx-auto px-4 sm:px-6 lg:px-8'>
        <div className='flex items-center justify-around gap-8 p-10 h-16'>
          ...
          <div className='hidden md:block'>
            <div className='ml-10 flex space-x-4'>
                {navlinks.map((link, index) => (
                  <NavLink key={index} to={link.href} className={({isActive}) => {
                   return 'text-primary font-semibold text-1xl hover:border-b-2 duration-100 cursor-pointer'
                  }} 
                  >{link.title}
                  </NavLink>
                ))}
          ...
          {/*text-primary font-semibold text-1xl hover:border-b-2 duration-100 cursor-pointer*/}
          {/* button de menu hamburguer */}
        ...
        <div className='md:hidden duration-[450ms]'>
            <div className='ox-2 pt-2 pb-3 space-y-1 sm:px-3 grid grid-cols-1 '>
              {navlinks.map((link, index) => (
                 <NavLink key={index} to={link.href} className={({isActive}) => {
                  return 'text-primary font-semibold text-1xl hover:border-b-2 duration-100 cursor-pointer' +
                  (isActive ? 'text-primary hover:border-b-2' : 'bg-white');
                 }} >{link.title}</NavLink>
              ))}
            </div>
          </div>
